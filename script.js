...
const seeContentsBtn = document.querySelector('.seeContentsBtn');
...
function showMoreBtn() {
  const morebtn = divCover.querySelector('.seeContentsBtn');
...
function updateGridColumns() {
  ...
  if (windowWidth < 800) {
    ...
    seeContentsBtn.style.display = 'block';

    if (seeContentsBtn.parentNode === divCover) {
      seeContentsBtn.addEventListener('click', (e) => {
        e.preventDefault();
        showMoreBtn();
      });
    ...
    section.innerHTML = '';
    seeContentsBtn.style.display = 'none';
